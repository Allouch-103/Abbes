...
DOF per leg~\cite{sias1990}. Each arm uses three DOF, and the head two, as detailed in
...
the servos through a \textbf{PCA9685} 16-channel PWM driver board on the
I\textsuperscript{2}C bus (at address 0x40). In the current build,
sixteen of the eighteen joints are actuated --- the ten leg joints and
the six arm joints --- which exactly fills the sixteen channels of one
board; the two head joints (\texttt{head\_yaw} and \texttt{camera\_pitch})
are part of the kinematic model but are not actuated in this version. The
same I\textsuperscript{2}C bus connects the \textbf{MPU6050} IMU. On the
host (laptop) side, a micro-ROS \emph{agent}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    \node[drv, right=20mm of esp] (pca0) {PCA9685\\\scriptsize 0x40};
    \node[sen, below=12mm of esp] (imu) {MPU6050\\\scriptsize IMU};
    \node[box, right=16mm of pca0, fill=gray!10] (s0) {16 servos\\\scriptsize legs + arms};
    \node[pwr, below=12mm of pca0] (pwr) {Battery $\rightarrow$ DC/DC regulator};

    \draw[<->] (host) -- node[above,font=\scriptsize]{WiFi/UDP} (esp);
    \draw[->] (esp) -- node[above,font=\scriptsize]{I\textsuperscript{2}C} (pca0);
  ...
  \caption{Electronic architecture of Abbes: the ESP32 micro-ROS node
           drives the 16 actuated servos through a single PCA9685 board
           and reads the MPU6050, communicating with the ROS\,2 host over
           WiFi. Dashed red lines indicate power distribution.}
  \label{fig:hwarch}
\end{figure}
...
architecture is built around an ESP32 that drives the actuated servos
through a PCA9685 board and joins ROS\,2 as a micro-ROS node. The next chapter
