...
───────────────────
  ...
  Subscribes:
    ...
    /joint_nominal (std_msgs/Float32MultiArray)    — θ_nominal from ZMP planner
...
TOPICS (identical to old PID node):
  ...
  IN:  /joint_nominal std_msgs/Float32MultiArray   ← NEW (from ZMP planner)
...
ANKLE_PITCH_IDX    = 4       # index in the 18-float array — left ankle pitch
ANKLE_ROLL_IDX     = 5       # index in the 18-float array — left ankle roll
ANKLE_PITCH_IDX_R  = 10      # right ankle pitch
ANKLE_ROLL_IDX_R   = 11      # right ankle roll
...
class BalanceController(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(
            ...
            self._imu_cb, 10)
        self.create_subscription(
            Float32MultiArray, '/joint_nominal',
            self._nominal_cb, 10)
